...
// FNV-1a Hash Logic (C++ Backend se match karta hua)
...
function App() {
  ...
  const [keysOnRing, setKeysOnRing] = useState([]);
  
  // NAYA: Nodes ab React State mein hain taaki hum unhe zinda/murda kar sakein
  const [nodes, setNodes] = useState([
    { name: "8080", angle: 0, color: "#06b6d4", isAlive: true }, // cyan
    { name: "8082", angle: 120, color: "#d946ef", isAlive: true }, // fuchsia
    { name: "8084", angle: 240, color: "#ef4444", isAlive: true }  // red
  ]);
  ...
  // THE MAGIC ALGORITHM: Zinda nodes mein se Asli Malik dhoondhna
  const getOwnerNode = (keyAngle) => {
    const activeNodes = nodes.filter(n => n.isAlive);
    if (activeNodes.length === 0) return { color: "#64748b", name: "Cluster Offline" }; // slate-500
  ...
  const handlePut = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        setKeysOnRing(prev => {
          const filtered = prev.filter(k => k.name !== currentKey);
          return [...filtered, { name: currentKey, angle: calculatedAngle }];
        });
      }
      setKeyInput("");
      setValInput("");
    } catch (err) {
      addLog("[CRITICAL] Backend unreachable. Is the C++ server running?");
    }
  };
  ...
  const handleInject = async () => {
    addLog("[CHAOS] 🚀 Injecting 50 random keys into cluster...");
    for(let i=0; i<50; i++) {
      const randomKey = `user_${Math.floor(Math.random() * 9999)}`;
      ...
      fetch('http://127.0.0.1:8080/put', {
        ...
        if (data.status === "success") {
          setKeysOnRing(prev => [...prev.filter(k => k.name !== randomKey), { name: randomKey, angle: calculatedAngle }]);
        }
      }).catch(err => {});
    }
  };
  ...
  const handleRebalance = async () => {
    ...
    try {
      ...
      addLog("[CRITICAL] Rebalance failed to execute.");
    }
  };

  // NAYA: Node ko kill karne ka UI function
  const toggleNodeStatus = (targetName) => {
    setNodes(prev => prev.map(n => {
      if (n.name === targetName) {
        const isNowAlive = !n.isAlive;
        addLog(`[GOSSIP] Node ${targetName} marked as ${isNowAlive ? "ALIVE" : "DEAD (Simulated)"}.`);
        return { ...n, isAlive: isNowAlive };
      }
      return n;
    }));
  };
  ...
  return (
    <div className="h-screen w-screen bg-slate-950 text-white flex overflow-hidden">
      ...
      {/* ⬅️ Left Panel */}
      <div className="w-1/4 border-r border-slate-800 p-6 flex flex-col bg-slate-900 shadow-2xl z-10 overflow-y-auto">
        ...
        <div className="flex flex-col gap-4 mt-8">
          ...
          <div className="flex gap-3">
            <button onClick={handlePut} className="bg-cyan-500 px-4 py-3 rounded text-slate-950 font-bold hover:bg-cyan-400 transition w-1/2">PUT</button>
            <button onClick={handleGet} className="bg-slate-700 px-4 py-3 rounded text-white hover:bg-slate-600 transition w-1/2 font-bold border border-slate-600">GET</button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 mt-8">
          ...
          <button onClick={handleInject} className="bg-purple-600/20 border border-purple-500/50 text-purple-300 px-4 py-3 rounded font-bold hover:bg-purple-600/40 transition">🚀 INJECT 50 KEYS</button>
          <button onClick={handleRebalance} className="bg-emerald-600/20 border border-emerald-500/50 text-emerald-400 px-4 py-3 rounded font-bold hover:bg-emerald-600/40 transition">⚖️ REBALANCE CLUSTER</button>
        </div>

        {/* ☠️ NODE STATUS (KILL SWITCHES) */}
        <div className="flex flex-col gap-3 mt-8">
          ...
          {nodes.map(node => (
            <div key={node.name} className="flex justify-between items-center bg-slate-950 p-3 rounded border border-slate-800">
              <div className="flex items-center gap-2">
                <span className={`w-3 h-3 rounded-full ${node.isAlive ? "animate-pulse" : ""}`} style={{ backgroundColor: node.isAlive ? node.color : "#334155" }}></span>
                <span className={`font-mono ${node.isAlive ? "text-white" : "text-slate-500 line-through"}`}>Node {node.name}</span>
              </div>
              <button 
                onClick={() => toggleNodeStatus(node.name)}
                className={`text-[10px] font-bold px-3 py-1 rounded border ${node.isAlive ? "border-red-500/50 text-red-400 hover:bg-red-900/30" : "border-green-500/50 text-green-400 hover:bg-green-900/30"}`}
              >
                {node.isAlive ? "☠️ KILL" : "➕ REVIVE"}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="w-96 h-96 border-[3px] border-slate-700 rounded-full relative flex items-center justify-center z-10 shadow-[inset_0_0_50px_rgba(0,0,0,0.5)] bg-slate-900/50 backdrop-blur-sm">
          ...
          {nodes.map((node, i) => {
            ...
            return (
              <div 
                key={`node-${i}`}
                style={{ transform: `translate(${x}px, ${y}px)`, backgroundColor: node.isAlive ? node.color : "#334155" }}
                className={`w-10 h-10 rounded-full absolute flex items-center justify-center text-[11px] font-bold border-4 border-slate-900 z-20 text-white transition-colors duration-500 ${node.isAlive ? "shadow-[0_0_20px_rgba(255,255,255,0.2)]" : "opacity-50"}`}
              >
                {node.name}
              </div>
            );
          ...
          {keysOnRing.map((item, i) => {
            ...
            return (
              <div 
                ...
                className="w-3 h-3 rounded-full absolute z-15 border border-white group cursor-pointer transition-all duration-500"
              >
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-cyan-500 text-[10px] mt-1 font-bold">KEYS: {keysOnRing.length}</span>
          </div>
        </div>
      </div>
